...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const fetchWeeklyReportCount = async (projectId) => {
      try {
        const response = await axios.get(`https://projectpulsecloned.onrender.com/ShowWeeklyReports/${projectId}`);
    ...
    const fetchFileCount = async (projectId) => {
      try {
        const response = await axios.get(`https://projectpulsecloned.onrender.com/ShowFilesToStudent/${projectId}`);
    ...
    const fetchData = async () => {
      try {
        const result = await axios.get(`https://projectpulsecloned.onrender.com/ShowProjectsByEmail/${user.email}`);
        ...
        const progressData = await Promise.all(projectsData.map(async (project) => {
          try {
            const [progressResponse, reportCount, fileCount] = await Promise.all([
              axios.get(`https://projectpulsecloned.onrender.com/api/ShowProgress/${project._id}`),
  ...
  const handleDetailClick = async (project) => {
    ...
    try {
      ...
      const response = await axios.get(`https://projectpulsecloned.onrender.com/ShowTeams`);
