...
const FacultyProject = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const result = await axios.get(`https://projectpulsecloned.onrender.com/ProjectDetailByFaculty/${user.email}`);
        const projects = result.data;

        // Fetch progress for each project
        const projectsWithProgress = await Promise.all(projects.map(async (project) => {
          try {
            const progressResponse = await axios.get(`https://projectpulsecloned.onrender.com/api/ShowProgress/${project._id}`);
            return {
              ...project,
  ...
  const handleCommentSubmit = () => {
    ...
    axios.put(`https://projectpulsecloned.onrender.com/EditProjects/${selectedProject._id}`, selectedProject)
      .then(() => {
