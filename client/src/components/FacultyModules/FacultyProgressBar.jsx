...
const FacultyProgressBar = ({ projectId, onUpdateProgress }) => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const response = await axios.get(`https://projectpulsecloned.onrender.com/api/ShowProgress/${projectId}`);
        if (response.data) {
  ...
  const handleUpdateProgress = async () => {
    try {
      const response = await axios.post('https://projectpulsecloned.onrender.com/api/AddProgressForStudent', { projectId, progress });
